...
const atmCheck = pinCheck => {
    let check4digits = pinCheck.match(/^\b\d{4}\b$/)
    let check6digits = pinCheck.match(/^\b\d{6}\b$/)
    if (pinCheck == check4digits || pinCheck == check6digits) {
        return true
    }
    return false
}
console.log(atmCheck('123456'))
