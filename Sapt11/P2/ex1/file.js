...
const getAccount = myData.filter(ifActive => ifActive.active === true);
...
const distances = [
  ...
  const convertElem = distances.map( elem => elem
    .distance * 0.621371 + ' miles');
  console.log(convertElem)

  const returnCertainMiles = distances
    .filter( elem => elem.distance < 10000);
  console.log(returnCertainMiles)

  const returnSum = distances
    .reduce((acc, item) =>{
...
const getData = releases.map(elem => ({
    id: elem.id,
    title: elem.title
}));
console.log(getData)
...
const filterMovies = newReleases
...
const getElements = movieLists
    .map( elem => elem.videos)
        .flat();

const getList = getElements.map( items => {
    let myImg = items.boxarts.find(myBoxart => myBoxart.width === 150);
        return {
            id : items.id,
            title : items.title,
            img : myImg,
